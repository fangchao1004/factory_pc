...
export function RenderEngine({ jsonlist, userList, currentUserId, currentPageIndex, scaleNum = 1 }) {
    ...
    // }, [])
    const changeComponetsValue = useCallback((index, value) => {
    ...
    const componentsRender = useCallback((item, index) => {
        ...
        switch (item.type) {
            case 'input':
                return <input key={index} {...item.attribute} onChange={(e) => { changeComponetsValue(index, e.target.value); }} />
            case 'textarea':
                return <textarea key={index} {...item.attribute} onChange={(e) => { changeComponetsValue(index, e.target.value); }} />
            case 'daterange':
                return <DatePicker.RangePicker placeholder='' size='small' key={index} {...item.attribute} format="YYYY年MM月DD日 HH时mm分" showTime={{ format: 'HH时mm分' }}
                    value={item.attribute.value.length > 1 ? [moment(item.attribute.value[0]), moment(item.attribute.value[1])] : null}
                    onChange={(_, dateString) => { changeComponetsValue(index, changeMomentFormat(dateString)) }}
                />
            case 'datepicker':
                return <DatePicker placeholder='' size='small' key={index} {...item.attribute} format="YYYY年MM月DD日 HH时mm分" showTime={{ format: 'HH时mm分' }}
                    value={item.attribute.value ? moment(item.attribute.value) : null}
                    onChange={(_, dateString) => { changeComponetsValue(index, changeMomentFormat(dateString)) }}
                />
            case 'checkbox':
                return <Checkbox key={index} {...item.attribute} checked={item.attribute.value ? true : false} onChange={(e) => {
                    changeComponetsValue(index, e.target.checked);
                }} />
            case 'checkboxgroup':
                return <Checkbox.Group key={index} {...item.attribute} onChange={(checkedValues) => { changeComponetsValue(index, checkedValues); }} />
            case 'select':
                return <Select
                    ...
                    onChange={(value) => { changeComponetsValue(index, value); }}
...
function changeMomentFormat(timelistOrStr) {
    if (typeof timelistOrStr == 'string') {
        return timelistOrStr.replace(/年/g, "-").replace(/月/g, "-").replace(/日/g, "").replace(/时/g, ":").replace(/分/g, "")
    }
