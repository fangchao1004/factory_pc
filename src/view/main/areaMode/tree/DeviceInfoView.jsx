...
class DeviceInfoView extends Component {
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.deviceObj) { this.init(nextProps.deviceObj); }
        else { this.setState({ showTable: false }) }
    ...
    render() {
        const columns = [{
            title: <div style={{ color: '#3080fe' }}>{this.state.deviceObj ? this.state.deviceObj.title : ''}</div>,
            children: [
                {
                    title: '时间',
                    dataIndex: 'checkedAt',
        ...
        return (
            <div>
                {this.state.showTable ?
                    <div>
                        <h2 style={{ borderLeft: 4, borderLeftColor: "#3080fe", borderLeftStyle: 'solid', paddingLeft: 5, fontSize: 16 }}>设备巡检记录</h2>
                        <Table
                            bordered
