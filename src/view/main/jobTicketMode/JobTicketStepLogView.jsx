...
export default function JobTicketStepLogView({ record, visible, onCancel }) {
    ...
    const init = useCallback(async () => {
        if (record) {
            ...
            if (res.data.code === 0) {
                let tempList = res.data.data.map((item) => { return { time: item.time, user_name: item.user_name, step_des: item.step_des, remark: item.remark } })
            ...
            {steplist.map((item, index) => {
                return <Timeline.Item key={index}>
                    <div>
                        ...
                        <Tag color={'#ff7a45'} ><Icon style={{ marginRight: 5 }} type="user" />{item.user_name}</Tag>
