...
export default class BugAboutMeViewNew extends Component {
    ...
    autoFixHandler = (list) => {
        ...
        if (JSON.parse(localUserInfo).permission && JSON.parse(localUserInfo).permission.indexOf('3') !== -1 && user_major_id.length > 0) { ///有维修权限
            let needList = [];
            ...
            if (needList.length > 0) {
                ...
                HttpApi.obs({ sql }, (res) => {
                    if (res.data.code === 0) {
                        let sql = `update bugs set status = 1 where id in (${needList.join(',')}) `;
                        HttpApi.obs({ sql }, (res) => {
                            if (res.data.code === 0) {
